...
for i in range(size(tt)):
    ...
    # Calculate C and N
        ...
        Fnfix_c = Fnfix_max*(Csto/(Csto+Kc))*1# C usage in N fixation mol C m-3
        ...
        dndt = Fnfix -Fbio*((Ync+Nsto)/(1+Csto))# nitrogen changing rate d-1
        ...
        dcdt = -Fnfix*1 - Fres/(1*QC)-Fbio# carbon concentration in cell molC molC-1
        dndt = Fnfix-Fbio*((Ync+Nsto)/(1+Csto))# nitrogen concentration in cell molN molC-1
    ...
    if mod(i,120)==0 and i>0:
        ...
        #print((1+Csto)/(Ync+Nsto),(Csto/Nsto),Fnfix)
    #t2 = time()
    #print(i, round(t2-t0,2))
    #t0 = time() 
...
SC=array([["","C fate","C fate fraction"
              ...
              ["Csto1",sum(CSTO)*dt,sum(CSTO)*dt/sum(TC)*dt],
              ["bio",sum(bio)*dt,sum(bio)*dt/sum(TC)*dt],
              ["cNfix",sum(cNfix)*dt,sum(cNfix)*dt/sum(TC)*dt],
              ["res",sum(res)*dt,sum(res)*dt/sum(TC)*dt],
         ])
SN=array([["","N fate","N fate fraction"
],
              ["Nsto1",sum(NSTO)*dt,sum(NSTO)*dt/sum(TN)*dt],
              ["bio",sum(bion)*dt,sum(bion)*dt/sum(TN)*dt],
         ])
...
savefig('figure_O2_H10614.png',dpi=300)
...
#savetxt('mu_1min_H2_0207.csv', mu_m, delimiter=",", fmt='%.8e')
#savetxt('SC1_1min_H2_0522.csv', SC, delimiter="," ,fmt ='%s')
#savetxt('SN1_1min_H2_0522.csv', SN, delimiter=",", fmt="%s")
#savetxt('Nfix_1min_H2_0907.csv', Nfix, delimiter=",", fmt="%s")
